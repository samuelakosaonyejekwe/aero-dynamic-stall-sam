...
bullet("Closure. Integrating the reconstructed surface C_p must return the C_L the "
       "reconstruction was given (Γ = ½ C_L U c). Measured with the dynamic-stall "
       "vortex OFF, which is the only configuration in which the residual is a "
       "closure error at all — a free vortex near the body exerts a real force on "
       "it, so with the vortex present part of the residual is its induced lift and "
       "is published separately as DSV_induced_lift_dCL "
       f"({_mA_c['DSV_induced_lift_dCL']} for Case A). At peak incidence the closure is "
...
bullet("Dynamic-stall vortex — both of its properties are DERIVED, neither is "
       "chosen. Its circulation is Kutta–Joukowski on its own normal-force "
       "contribution, Γ_v = ½ C_N^v U c, the same relation the bound sheet uses, so "
       "it carries exactly the C_N^v/C_N share of the circulation the lift implies "
       f"({_mA_c['DSV_circulation_over_Uc']} U c for Case A) and the circulation budget "
       "closes. Its core is the radius at which that circulation swirls at the edge "
       "speed of the shear layer that rolls it up, r_c = 0.6382 Γ_v/(2π V_e), with "
       "V_e read off the panel solution at the vortex centre with the vortex excluded "
       f"({_mA_c['DSV_core_radius_chords']} c, swirling at "
       f"{_mA_c['DSV_peak_swirl_over_U']} U). The core suction that follows is "
       f"Cp_DSV_core_min = {_mA_c['Cp_DSV_core_min']} (Case A) and "
       f"{_mB_c['Cp_DSV_core_min']} (Case B), against the −3 to −6 that dynamic-stall "
       "measurements report — landing there without having been fitted to it. The two "
       "chosen constants this replaces (1.4 C_N^v U c in a 0.16c core) gave −0.36, an "
       "order of magnitude too shallow, under a note claiming they could not be "
       "derived from anything this study ships. The derived core is small: r_c is "
       f"{_mA_c['DSV_core_radius_cells']} of a cell of the published field grid, so the "
       "FIELD does not resolve it (published as DSV_core_radius_cells). The depth "
       "above is exact because it is evaluated at the vortex centre rather than off "
       "the grid, and the vortex is still represented correctly in the field because "
       "its vorticity is added in closed form rather than differenced — integrating "
       "the published vorticity around the core returns its circulation to 0.4 %.")
...
for _j, (title, pref) in enumerate(groups, start=1):
    ...
    if pref == "contour_vorticity_":
        P("The dynamic-stall vortex is a roll-up of upper-surface boundary-layer "
          ...
          "low-pressure core, whose depth is derived rather than chosen (§4.9). It "
          "does NOT reverse the flow at the wall beneath itself, and this paragraph "
          "used to say it did: the vortex induces "
          f"{_mA_c['DSV_induced_at_wall_over_U']} of the free stream upstream there "
          f"against a local {_mA_c['DSV_edge_speed_over_U']} "
          "(DSV_induced_at_wall_over_U and DSV_edge_speed_over_U in metrics_*.csv), "
          "so it cannot turn the flow over. The published fields agree: the few "
          "reversed cells they contain sit under the LEADING EDGE on the pressure "
          "side, which is ordinary stagnation-region turning, while the vortex is at "
          "x/c = 0.79. What the vortex does have is the core suction of §4.9 and the "
          "vorticity signature below, both derived and both landing where "
          "measurement puts them. The reported loads come "
